...
1. `ls .mikado/*.md 2>/dev/null` — if empty, stop with: "No Mikado goal found. Run `/mikado <goal>` (or your project's wrapper, e.g. `/mikado-myproject <goal>`) first."
...
7. Any known-flake / environment-quirk list from the project's wrapper skill, if one exists.
...
- `mikado` (and any project wrapper skill) — start the goal this skill advances
