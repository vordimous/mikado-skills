...
1. `ls .mikado/*.md 2>/dev/null` — if empty, stop with: "No Mikado goal found. Run `/mikado <goal>` or `/mikado-x1 <goal>` first."
...
7. Any known-flake / environment-quirk list from the wrapper skill (`mikado-x1` in the x1 repo codifies these).
...
- `mikado`, `mikado-x1` — start the goal this skill advances
